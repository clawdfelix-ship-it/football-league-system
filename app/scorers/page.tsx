  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch('/api/stats/scorers', { cache: 'no-store' });
        if (!res.ok) throw new Error(`Request failed: ${res.status}`);
        const data = (await res.json()) as { scorers: ScorerRow[] };
        if (!active) return;
        setRows(Array.isArray(data.scorers) ? data.scorers : []);
      } catch (e) {
        if (!active) return;
      ...
      <div className="min-h-screen bg-white text-slate-900 font-sans">
        ...
        <main className="max-w-4xl mx-auto px-6 py-10">
          <div className="rounded-2xl border border-slate-200 overflow-hidden">
            <div className="bg-slate-50 px-6 py-4 font-semibold">
              {t('排行榜', 'Leaderboard')}
            </div>
            {loading ? (
              <div className="px-6 py-10 text-sm text-slate-600">{t('載入中...', 'Loading...')}</div>
            ) : error ? (
              <div className="px-6 py-10 text-sm text-red-700">{error}</div>

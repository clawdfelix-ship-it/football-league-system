                  ...
                  recentResults.map((match) => (
                    ...
                    >
                      <div className="text-left w-1/3">
                        <div className="font-bold text-slate-900">{match.homeTeam}</div>
                        <div className="text-xs text-slate-500">{match.date ? new Date(match.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) + ' (' + new Date(match.date).toLocaleDateString('en-GB', { weekday: 'short' }) + ')' : ''}</div>
                      </div>
                      <div className="text-center w-1/3">
                    ...
                    <div key={item.id} className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-xs font-medium text-slate-500 bg-slate-200 px-2 py-1 rounded">
                          {new Date(item.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) + ' (' + new Date(item.date).toLocaleDateString('en-GB', { weekday: 'short' }) + ')'} {new Date(item.date).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
